            ...
            <form action="/todos/{{$todo->id}}" method="post" class="mt-3">
                ...
                <div class="input-group">
                    <div class="input-group-text bg-white p-0">
                        <input type="color" class="form-control form-control-color border-0" name="color" title="Escolha uma cor" value="{{$todo->color ?? '#ffffff'}}">
                    </div>
                    <input type="text" class="form-control" name="title" placeholder="O que fazer?" value="{{$todo->title}}" required>
